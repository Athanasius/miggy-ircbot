...
sub _poco_weeble_connect_done {
  ...
  if (defined $connection) {
    ...
    DEBUG and warn "response: ", Dumper($response), "\n";
    DEBUG and warn "heap: ", Dumper($heap), "\n";
    DEBUG and warn "factory: ", Dumper($heap->{factory}), "\n";
    DEBUG and warn "cm: ", Dumper($heap->{cm}), "\n";
...
#      SSLPlaintextHandle =>
    ...
    if ( $request->scheme eq 'http' ) {
      ...
      # XXX: Need to change $new_wheel->[Driver] to something that will
      #      correctly account for the size difference between plaintext
      #      and encrypted data from the actual socket.
    }
