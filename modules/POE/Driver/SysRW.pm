...
sub get {
  ...
  my $buffer = '';
  my $result = sysread($handle, my $b = '', $self->[BLOCK_SIZE]);
  while ($result) {
    $buffer .= $b;
    my @loop_vectors = ("", "", "");
    my %loop_filenos;
    my $fileno = fileno($handle);
    vec($loop_vectors[POE::Kernel::MODE_RD], $fileno, 1) = 1;
    $loop_filenos{$fileno} |= (1<< POE::Kernel::MODE_RD);
    SYSRW_DEBUG and warn "SysRW->get(): Checking select after sysread()\n";
    SYSRW_DEBUG and warn(
      "<fh> ,----- SELECT BITS IN ------\n",
      "<fh> | READ    : ", unpack('b*', $loop_vectors[POE::Kernel::MODE_RD]), "\n",
      "<fh> | WRITE   : ", unpack('b*', $loop_vectors[POE::Kernel::MODE_WR]), "\n",
      "<fh> | EXPEDITE: ", unpack('b*', $loop_vectors[POE::Kernel::MODE_EX]), "\n",
      "<fh> `---------------------------\n"
    );
    my $hits = CORE::select(
      my $rout = $loop_vectors[POE::Kernel::MODE_RD],
      my $wout = $loop_vectors[POE::Kernel::MODE_WR],
      my $eout = $loop_vectors[POE::Kernel::MODE_EX],
      0,
    );
    if (
      $hits < 0 and
      $! != POE::Kernel::EINPROGRESS and
      $! != POE::Kernel::EWOULDBLOCK and
      $! != POE::Kernel::EINTR and
      $! != 0                   # this is caused by SIGNAL_PIPE
    ) {
      SYSRW_DEBUG and warn("<fh> select error: $! (hits=$hits)");
    }
    SYSRW_DEBUG and warn(
      "<fh> ,----- SELECT BITS OUT -----\n",
    ...
    $result = sysread($handle, my $b = '', $self->[BLOCK_SIZE]);
  }
